...
def create_course(
    ...
    course = Course(
        ...
        progress_tracking_enabled=payload.progress_tracking_enabled,
...
def update_course(
    ...
    if payload.progress_tracking_enabled is not None:
        course.progress_tracking_enabled = payload.progress_tracking_enabled
    if payload.certification_enabled is not None:
        course.certification_enabled = payload.certification_enabled
...
@app.post("/api/courses/{course_id}/publish", response_model=CourseOut)
def publish_course(
    course_id: UUID,
    current: User = Depends(require_roles("admin", "manager")),
    db: Session = Depends(get_db),
...
def create_lesson(
    ...
    lesson = Lesson(
        ...
        reading_materials_json=payload.reading_materials,
        downloadable_resources_json=payload.downloadable_resources,
        order_index=payload.order_index,
    )
    db.add(lesson)
    db.commit()
...
@app.post("/api/modules/{module_id}/assessments", response_model=AssessmentOut)
...
@app.get("/api/modules/{module_id}/assessments", response_model=List[AssessmentOut])
def list_module_assessments(
    ...
    rows = db.scalars(
        select(Assessment).where(Assessment.tenant_id == current.tenant_id, Assessment.module_id == module_id)
    ).all()
    return rows


@app.post("/api/assessments/{assessment_id}/questions", response_model=AssessmentQuestionOut)
def add_assessment_question(
    assessment_id: UUID,
    payload: AssessmentQuestionCreateRequest,
    current: User = Depends(require_roles("admin", "manager")),
    db: Session = Depends(get_db),
):
    assessment = db.scalar(
        select(Assessment).where(Assessment.tenant_id == current.tenant_id, Assessment.id == assessment_id)
    )
    if not assessment:
...
def create_assignment(
    ...
    module = db.scalar(select(Module).where(Module.tenant_id == current.tenant_id, Module.id == payload.module_id))
    if not module:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Module not found")
...
@app.get("/api/modules/{module_id}/assignments", response_model=List[AssignmentOut])
def list_module_assignments(
    ...
    rows = db.scalars(
        select(Assignment).where(Assignment.tenant_id == current.tenant_id, Assignment.module_id == module_id)
    ).all()
    return rows
...
def enroll_user(
    ...
    course = db.scalar(select(Course).where(Course.tenant_id == current.tenant_id, Course.id == payload.course_id))
    if not course:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Course not found")
    ...
    existing = db.scalar(
        select(Enrollment).where(
            Enrollment.tenant_id == current.tenant_id,
...
@app.get("/api/courses/{course_id}/enrollments", response_model=List[EnrollmentOut])
def list_course_enrollments(
    ...
    rows = db.scalars(
        select(Enrollment).where(Enrollment.tenant_id == current.tenant_id, Enrollment.course_id == course_id)
    ).all()
    return rows
...
def list_users(
    ...
    rows = db.scalars(
        select(User).where(User.tenant_id == current.tenant_id).order_by(User.full_name)
    ).all()
    return rows


@app.post("/api/courses/{course_id}/feedback", response_model=CourseFeedbackOut)
def submit_course_feedback(
    course_id: UUID,
    payload: CourseFeedbackCreateRequest,
    current: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    course = db.scalar(select(Course).where(Course.tenant_id == current.tenant_id, Course.id == course_id))
...
@app.get("/api/courses/{course_id}/feedback", response_model=List[CourseFeedbackOut])
def list_course_feedback(
    course_id: UUID,
    current: User = Depends(require_roles("admin", "manager")),
    db: Session = Depends(get_db),
):
    rows = db.scalars(
        select(CourseFeedback).where(
            CourseFeedback.tenant_id == current.tenant_id,
...
@app.post("/api/courses/{course_id}/enroll-me", response_model=EnrollmentOut)
def self_enroll(
    course_id: UUID,
    current: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Allow any authenticated user to self-enroll in a published course."""
    course = db.scalar(select(Course).where(Course.tenant_id == current.tenant_id, Course.id == course_id))
    if not course:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Course not found")
...
@app.post("/api/courses/{course_id}/certificates", response_model=CertificateOut)
def issue_certificate(
    course_id: UUID,
    current: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Issue a certificate of completion to the current user for a course."""
    course = db.scalar(select(Course).where(Course.tenant_id == current.tenant_id, Course.id == course_id))
    if not course:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Course not found")
    if not course.certification_enabled:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="This course does not offer certification")
    existing = db.scalar(
        select(Certificate).where(Certificate.user_id == current.id, Certificate.course_id == course_id)
    )
    if existing:
        return existing
    cert_number = f"CERT-{datetime.utcnow().year}-{_uuid_module.uuid4().hex[:8].upper()}"
    cert = Certificate(
        tenant_id=current.tenant_id,
        user_id=current.id,
        course_id=course_id,
        certificate_number=cert_number,
        recipient_name=current.full_name,
        course_title=course.title,
        instructor_name=course.instructor_name or "Instructor",
        template_data_json={
...
@app.get("/api/certificates/me", response_model=List[CertificateOut])
def list_my_certificates(
    ...
    certs = db.scalars(
        select(Certificate).where(
            Certificate.tenant_id == current.tenant_id,
